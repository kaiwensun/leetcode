...
STARRED = {315, 321, 805, 913, 1199, 1713, 1819, 1923, 2040}
...
all_items_file_name = "all_items.md"
...
def gen_markdown(questions, solutions, title, markdown_type):
    ...
    def gen_markdown_questions(questions):
        ...
        if markdown_type == MarkdownType.MAIN_README:
            all_items_file_path = os.path.join(
                get_root_path(), "others", all_items_file_name)
            size_limit = (f"This table contains only the latest {MAIN_README_SIZE} items."
                          f"For a full list, check [{all_items_file_name}]({all_items_file_path}). "
                          "This is because GitHub currently limit blob rendering of a repo's README to around 500 KB of data."
                          )
        header = """
